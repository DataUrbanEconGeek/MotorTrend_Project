...
model_1 <- lm(mpg ~ disp + hp + wt, data = mtcars)
summary(model_1)
...
```{r}
model_2 <- lm(mpg ~ log(disp) + log(hp) + log(wt), data = mtcars)
summary(model_2)
...
```{r}
model_3 <- lm(mpg ~ log(disp) * log(hp) + log(wt), data = mtcars)
summary(model_3)
prediction_3 <- predict(model_3, newdata = mtcars)
pred_diff_3 <- with(mtcars, mpg - prediction_2)
mean(pred_diff_2 ^ 2)
```
